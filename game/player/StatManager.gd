...
func get_stat(p: Node, stat: String) -> float :
	match stat:
		"hp": 
			...
			return hp 
		"move_speed":
			var speed = p.base_move_speed + lerp(0.0, 225.0, (p.move_speed_level - 1.0) / 9.0)
			speed *= clamp((1 + p.speed_bonus * 0.03), 0.4, 1.6) 					# 20
			speed += p.EquipMoveSpeedFlat
			return speed 
		"luck":
			var luck = p.base_luck + lerp(0.0, 0.63, (p.luck_level - 1.0) / 9.0)
			luck *= (1 + p.luck_bonus * 0.08) 										# 20
			luck = clamp(luck, 0.1, 2.0)
			return luck 
		"magic":
			var magic = p.base_magic + lerp(0.0, 1.35, (p.magic_level - 1.0) / 9.0)
			magic *= clamp((1 + p.magic_bonus * 0.1), 0.0, 3)  						# 40
		...
		"spread":
			...
			if p.accuracy_bonus >= 0:
				spread_deg *= (1 - p.accuracy_bonus * 0.02)  						# 20
			else:
				spread_deg *= (1 - p.accuracy_bonus * 0.13)
			spread_deg = clamp(spread_deg, 0.1, 300.0)
			return spread_deg 
		"range": 
			var range_val = p.base_range + lerp(0.0, 360.0, (p.range_level - 1.0) / 9.0)
			range_val *= clamp((1 + p.range_bonus * 0.1), 0.4, 3.0) 					# 20
			return range_val 
		"fire_rate":
			...
			fire_rate *= p.EquipFireRateBoost
